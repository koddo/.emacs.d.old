  ...
  ("asdfasdf" (
		...
		"spanish")
	       ("4" (lambda () (interactive)
		      (org-ql-search "~/werk/Clojure.org" '(tags "drill" "drilltodo") :sort 'date)
		      (delete-other-windows)
		      )
		"clojure")
	       ("5" (lambda () (interactive)
		      (org-ql-search "~/werk/Clojure.org" '(and (tags "drill") (tags "read")) :sort 'date)
		      (delete-other-windows)
		      )
		"clojure drill read")
	       ("c" (lambda () (interactive)
		      (org-ql-search (org-agenda-files) '(and (tags "drill") (not (tags "english" "spanish" "humor"))) :sort '(date))
		      (delete-other-windows)
		      )
		"cards")
