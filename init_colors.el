...
(setq ym-org-agenda-tags-faces '((":birthday:" .  '(:foreground "red" :underline nil))
                                 ...
                                 ;; ("Scheduled:" . '(:foreground "red" :weight bold :underline nil))
                                 ))
...
(add-hook 'org-agenda-finalize-hook (lambda ()
                                      (let ((case-fold-search nil))
                                        ...
                                        (end-of-buffer)
                                        (search-backward-regexp (concat "^" ym-org-agenda-planning-header-now "$") nil t)
                                        (while (search-backward-regexp ym-org-todo-keywords-working-regexp nil t)
                                          (ym-add-overlay-to-line 'ym-org-agenda-dim-top-tag-face)
                                          )
                                        (beginning-of-buffer)
                                        ;; (while (search-forward-regexp "...... Scheduled:" nil t)
                                        (let* ((ym-tmp "........... Scheduled:")
                                               (ym-tmp-length (length ym-tmp)))
                                          (while (search-forward-regexp ym-tmp nil t)
                                            (let ((p (point)))
                                              (unless (string-match " 0:00." (substring-no-properties (thing-at-point 'line)))
                                                (ym-add-overlay '(:foreground "red" :weight bold) (- p ym-tmp-length) p)))
                                            ))
                                        (beginning-of-buffer)
