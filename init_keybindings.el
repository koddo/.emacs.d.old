...
(ym-define-key (kbd "<f3>") (lambda ()
                              (interactive)
                              (find-file org-default-notes-file)
                              (end-of-buffer)
                              (newline)
                              (forward-line -1)
                              ))   ; was (ym-org-datetree-find-today org-default-notes-file)
(ym-define-key (kbd "M-<f3>") 'ym-org-journal-view)
(ym-define-key (kbd "<f4>") 'ym-ledger-view)
(ym-define-key (kbd "M-<f4>") (lambda () (interactive)   (ym-ledger-view nil "--period-sort total --monthly --begin january -T '0'    reg Expences Liabilities")))
(ym-define-key (kbd "S-M-<f4>") (lambda () (interactive) (ym-ledger-view nil "--period-sort total --monthly --begin january -T '0' -n reg Expences Liabilities")))
...
(ym-define-key (kbd "<f7>") (lambda () (interactive) (register-list) (call-interactively 'jump-to-register)))
(ym-define-key (kbd "<S-f7>") (lambda () (interactive) (call-interactively 'window-configuration-to-register)))
(ym-define-key (kbd "<M-f7>") (lambda () (interactive) (call-interactively 'point-to-register)))
...
(add-hook 'org-agenda-mode-hook
          (lambda ()
            ...
            (define-key org-agenda-mode-map (kbd "r") 'ym-org-agenda-view-goto-random-next)
            (define-key org-agenda-mode-map (kbd "к") 'ym-org-agenda-view-goto-random-next)
...
(defun ym-org-day-view-aux-go-to-now-header ()
  ...
  (next-line)
  (beginning-of-line))
(defun ym-org-agenda-view-goto-random-next ()
  (interactive)
  (end-of-buffer)
  (let ((eof-line-number (line-number-at-pos)))
    (beginning-of-buffer)
    (search-forward-regexp ym-org-agenda-planning-header-next-actions nil t)
    (forward-line)
    (let* ((number-of-lines (- eof-line-number (line-number-at-pos)))
           (where-to-go (random* number-of-lines)))
      (forward-line where-to-go)
      )
    )
  )
;; -------------------------------------------------------------------
